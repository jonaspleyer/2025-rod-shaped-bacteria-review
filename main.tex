...
\begin{abstract}
    ...
    The individual behaviour of many bacteria can lead to emergent phenomena such as swarming and
    biofilm formation.
    A key question is how single-cell processes link to the emergence of collective phenomena.
    Mathematical and computational methods provide powerful tools which allow for an ansatz that
    can bridge this gap systematically.
    In this review we capture the current state of mathematical and computational approaches which
    are able to describe a wide range of biological processes, emphasizing flexible, mesoscopic
    models which are able to encompass the essential biological details that lead to a variety of
    collective effects.
    % Rod-shaped bacteria are among the most ubiquitous forms of life, encompassing a wide range of
    % species found in diverse environments.
    % Their elongated shape plays a crucial role in key biological processes, influencing motility,
    % growth, and collective behaviors such as swarming and biofilm formation.
    % A central question in understanding these systems is how the behavior of individual cells gives
    % rise to emergent collective phenomena.
    % Mathematical modeling provides a powerful framework to bridge this gap, linking single-cell
    % dynamics to population-level patterns.
    % In this review, we summarize the current state of mathematical approaches to rod-shaped
    % bacteria, with particular emphasis on mesoscopic models that capture essential biological
    % details while remaining tractable for studying emergent collective effects.
    % Our goal is to highlight how these models help reveal the fundamental principles connecting
    % microscopic cellular behavior to macroscopic organization in bacterial communities.
\end{abstract}

% keywords can be removed
\keywords{Single-Cell \and Mechanics \and Rod-Shaped \and Bacteria \and Modeling \and Mathematics}
...
\begin{itemize}
    ...
    \item \cite{Hu2015} "swimming properties of an E. coli-type model bacterium are investigated by mesoscale hydrodynamic simulations"
...
\begin{itemize}
    \item Segmentation~\cite{VanValen2016} omnipose~\cite{Cutler2022}, cellpose~\cite{Stringer2020}
    \item 3DeeCellTracker \cite{Wen2021}; Comparisons of Tracking Algorithms
